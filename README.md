...
|---|---|---|
| `tasks/release-train/control/` | Control: ultracode authors a native dynamic Workflow | `-p tasks/release-train/control` |
| `tasks/release-train/treatment/` | Treatment: same train, written as Trigger.dev | `-p tasks/release-train/treatment` |
| `tasks/fanout-audit/control/` | Control: ultracode authors a native dynamic Workflow | `-p tasks/fanout-audit/control` |
...
# fanout-audit treatment Oracle needs Trigger and Anthropic vars in .env
```
...
Paired model runs must use Claude Code in ultracode mode in both arms with the
same resolved author model, tool configuration, and authoring budget. For `fanout-audit`, set
`ANTHROPIC_MODEL` to the same Claude model release used by the control Workflow
children and retain both resolved run configs. Treat a missing Workflow call in
control, a model mismatch, or missing Trigger Run API evidence as an invalid
arm assignment rather than a model failure.
